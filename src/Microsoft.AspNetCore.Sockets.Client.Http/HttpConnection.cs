        ...
        private async Task DisposeAsyncCore(Exception exception = null)
        {
            ...
            try
            {
                if (!_disposed && _started)
                {
                    ...
                    // REVIEW: We could make not throwing part of the contract here.
                    try
                    {
                        await _transport.StopAsync();

                        // Wait for the transport to complete its side of the pipes
                        // REVIEW: We don't have to do this. But I think I do prefer that DisposeAsync only completes when everything is indeed complete
                        // We could just unref the pipe and assume it'll shut down. All our transports will be tested to ensure they properly shut down though.
